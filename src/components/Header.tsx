  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-background px-4 md:px-6 shadow-sm">
      ...
      <Link href={role === "foreman" ? "/foreman/dashboard" : role === "admin" ? "/admin/dashboard" : "/"} className="flex items-center gap-2 text-lg font-semibold md:text-base flex-shrink-0">
        ...
        <span className="font-bold text-lg text-primary truncate sm:hidden">QADR</span>
      </Link>
      <div className="ml-auto flex items-center gap-4">
        {isLoading && <Loader2 className="h-5 w-5 animate-spin text-primary" />}
        
        <div className="flex items-center gap-1 text-sm text-muted-foreground" title="Current time">
